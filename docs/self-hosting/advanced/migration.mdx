...
functionality (available in this AGPL fork without any license key).
...
**🚀 New Features (available in this AGPL fork without license):**
...
<Warning>
  **Upgrading to 3.0+ (or this AGPL fork)**

With **Formbricks 3.0**, upstream moved some advanced capabilities (SSO, user identification, cluster support) into their **Enterprise Edition**. **This AGPL fork includes all of those features** under AGPLv3 with no license key — see the fork [LICENSE](/self-hosting/advanced/license).
...
Formbricks v2.7 includes all the features and improvements developed by the community during Hacktoberfest 2024. Additionally, this release introduces an advanced team-based access control system, which is available in this AGPL fork (no license required).
...
<Info>
  Upstream's Enterprise Edition used to require a license key. This AGPL fork has removed license gating entirely — no key is required. See the fork [license page](/self-hosting/advanced/license).
</Info>
