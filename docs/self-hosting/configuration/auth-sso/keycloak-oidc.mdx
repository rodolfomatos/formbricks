...
<Note>Keycloak OIDC is available in this AGPL fork without a license key (upstream: [Enterprise Edition](/self-hosting/advanced/license))</Note>
...
- A self-hosted Formbricks instance (this AGPL fork — no license key required; upstream: [Enterprise license](/self-hosting/advanced/license)).
...
<Note>
  The Teams feature is available in this AGPL fork without any license add-on. If you don't see the Teams section in Organization Settings, contact your instance administrator.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Users see 'No membership found' after login">
    ...
    - Confirm the team has been granted the appropriate role in the AGPL fork (no license required — Teams is always available).
    - Ensure the team access is enabled in Organization Settings.
  </Accordion>
  ...
  <Accordion title="OIDC login button does not appear">
    ...
    - Ensure your Formbricks instance is running (this AGPL fork requires no license key).
